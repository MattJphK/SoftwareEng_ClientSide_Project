...
$result4 = [];
$moviescore = "";
if (isset($_POST['score_submit']) && isset($_POST['moviescore'])) {
    try {
        $sql4 = "SELECT * FROM review WHERE moviescore = :moviescore";
        $moviescore = $_POST['moviescore'];
        $statement4 = $connection->prepare($sql4);
        $statement4->bindParam(':moviescore', $moviescore, PDO::PARAM_INT);
        $statement4->execute();
...
if (isset($_POST['score_submit'])) {
    ...
    <?php } else { ?>
        > No Reviews found for <?php echo escape($_POST['moviescore']); ?>
    <?php }
} ?>
    <h2>Find reviews based on score</h2>
    <form method="post">
        <label for="score">Score</label>
        <input type="text" id="score" name="score" placeholder="1-10">
        <input type="submit" name="score_submit" value="View Results">
    </form>
    <a href="admin.php">Back to home</a>
